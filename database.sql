...
-- -----------------------------------------------------------------------------
-- -----------------------------------------------------------------------------
-- Upgrade: ensure attendance_logs exists safely and has snapshot column
-- -----------------------------------------------------------------------------
SET @db := DATABASE();

-- Check if snapshot column exists
SELECT COUNT(*) INTO @snap_missing 
FROM information_schema.COLUMNS
WHERE TABLE_SCHEMA = @db 
  AND TABLE_NAME = 'attendance_logs' 
  AND COLUMN_NAME = 'snapshot';

-- Safely add snapshot if missing
SET @sql_add_column := IF(@snap_missing = 0,
    'ALTER TABLE attendance_logs ADD COLUMN snapshot LONGTEXT AFTER scan_type;',
    'SELECT "Column snapshot already exists" AS msg;'
);
