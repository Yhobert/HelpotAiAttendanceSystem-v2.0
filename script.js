...
function serverRowToLocal(row) {
    ...
    return {
        ...
        logIn: row.logIn || row.log_in_time || '',
        logOut: row.logOut || row.log_out_time || '',
...
async function pushScanToServer(row, attendanceMode, snapshotDataUrl) {
    ...
    try {
        const r = await fetch('api/save_log.php', {
            method: 'POST',
...
async function pullServerLogsAndMerge() {
    ...
    try {
        const r = await fetch('api/get_logs.php?limit=500', { credentials: 'same-origin' });
        const j = await r.json().catch(() => ({}));
...
async function tick(){
    ...
    if(video.readyState >= HTMLMediaElement.HAVE_CURRENT_DATA){
        ...
        // Run QR decode every 2nd frame — cuts CPU/GPU load and reduces lag on repeated scans.
        scanFrameCounter++;
        const doDecode = (scanFrameCounter % 2 === 0);
...
function saveLogItem(d) {
    ...
    void pushScanToServer(row, mode, d.snapshot);
    ...
    if (d.snapshot) {
        ...
        saveSnapshotToDisk(d.snapshot, label)
            ...
            .catch(() => {})
            .finally(() => {
                if (sheetSyncPayload) void syncAttendanceToGoogleSheets(sheetSyncPayload);
            });
        d.snapshot = null;
    } else if (sheetSyncPayload) {
...
async function syncAttendanceToGoogleSheets(payload) {
    ...
    try {
        ...
        const r = await fetch('api/apps_script_proxy.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
clearLogBtn.addEventListener('click', async () => {
    ...
    if (typeof window.helportIsServerAuthed === 'function' && window.helportIsServerAuthed()) {
        try {
            const r = await fetch('api/clear_logs.php', { method: 'POST', credentials: 'same-origin' });
            const j = await r.json().catch(() => ({}));
            if (!r.ok || !j.success) {
...
exportCsvBtn.addEventListener('click', async () => {
    ...
    if (typeof window.helportIsServerAuthed === 'function' && window.helportIsServerAuthed()) {
        try {
            const r = await fetch('api/export_csv.php', { credentials: 'same-origin' });
            if (!r.ok) {
