...
// Store scan logs in IndexedDB (removes the artificial 200-log limit from localStorage).
// Note: localStorage has a hard size limit; IndexedDB is designed for large datasets.
const DB_NAME = 'helportai_attendance';
const DB_VERSION = 1;
const LOG_STORE = 'logs';
const MIGRATION_FLAG_KEY = 'qr-log-idb-migrated-v1';
    ...
    _dbPromise = new Promise((resolve, reject) => {
        ...
        req.onupgradeneeded = () => {
            ...
            if (!db.objectStoreNames.contains(LOG_STORE)) {
                const store = db.createObjectStore(LOG_STORE, { keyPath: 'employeeDateKey' });
...
async function migrateLogsFromLocalStorageOnce() {
    ...
    await new Promise((resolve, reject) => {
        ...
        for (const r of arr) {
            const employeeKey = String(r.employeeKey || '').toLowerCase().trim();
            const dateKey = String(r.dateKey || '').trim();
            if (!employeeKey || !dateKey) continue;
            const employeeDateKey = `${employeeKey}__${dateKey}`;
            const row = { ...r, employeeKey, dateKey, employeeDateKey };
            delete row.snapshot; // never persist snapshots
            store.put(row);
        }
...
async function idbGetAllLogsSorted() {
    const db = await openLogDb();
...
async function idbGetRow(employeeDateKey) {
    const db = await openLogDb();
    return await new Promise((resolve, reject) => {
        const tx = db.transaction(LOG_STORE, 'readonly');
        const store = tx.objectStore(LOG_STORE);
        const req = store.get(employeeDateKey);
        req.onsuccess = () => resolve(req.result || null);
        req.onerror = () => reject(req.error);
    });
}

async function idbPutRow(row) {
    const db = await openLogDb();
    return await new Promise((resolve, reject) => {
        const tx = db.transaction(LOG_STORE, 'readwrite');
        const store = tx.objectStore(LOG_STORE);
...
async function idbUpdateSaved(employeeDateKey, savedPath) {
    const row = await idbGetRow(employeeDateKey);
    if (!row) return;
    row.saved = savedPath || row.saved || '';
    await idbPutRow(row);
}
...
// 🗣️ Speak employee nickname – build message from current scan so correct person is announced
const _speakQueue = [];
let _speakVoicesReady = false;
...
function speakEmployeeAction(qrText, action = "logged in") {
    ...
    if (voices.length > 0) {
        ...
        _speakQueue.push(messageText);
        if (!_speakVoicesReady) {
            _speakVoicesReady = true;
            window.speechSynthesis.onvoiceschanged = () => {
                function speakNext() {
                    if (_speakQueue.length === 0) return;
                    const textToSpeak = _speakQueue.shift();
                    window.speechSynthesis.cancel();
                    const msg = new SpeechSynthesisUtterance(textToSpeak);
                    msg.lang = "en-PH";
                    msg.pitch = 1.25;
...
async function saveLogItem(d) {
    await migrateLogsFromLocalStorageOnce();
    ...
    let row = await idbGetRow(employeeDateKey);

    // 5-minute cooldown per employee (same person) to avoid log in/out at same time
    const FIVE_MIN_MS = 5 * 60 * 1000;
    if (row && (Date.now() - (row.timestamp || 0)) < FIVE_MIN_MS) {
        if (status) status.textContent = 'Please wait 5 min before scanning again';
        return;
    }
    if (!row) {
        row = {
    ...
    if (d.snapshot) {
        ...
        saveSnapshotToDisk(d.snapshot, label)
            .then(savedPath => {
                if (!savedPath) return;
                void idbUpdateSaved(employeeDateKey, savedPath).then(() => updateLatestSnapshotView());
            })
            .catch(() => {});
...
async function renderLog() {
    await migrateLogsFromLocalStorageOnce();
    const log = await idbGetAllLogsSorted();
...
async function updateLatestSnapshotView(){
...
clearLogBtn.addEventListener('click', () => {
    if (!isAdmin()) return;
    void idbClearLogs().then(() => renderLog());
});
    ...
    void (async () => {
        ...
        log.forEach(r => {
            ...
            const row = [
                ...
                `"${nickname.replace(/"/g, '""')}"`,
                `"${name.replace(/"/g, '""')}"`,
                `"${(r.date || '').toString().replace(/"/g, '""')}"`,
                `"${(r.logIn || '').toString().replace(/"/g, '""')}"`,
                `"${(r.logOut || '').toString().replace(/"/g, '""')}"`,
                `"${(r.saved || '').replace(/"/g, '""')}"`
            ].join(',');
